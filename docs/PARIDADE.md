...
| Linha do tempo da análise sênior (fases + contador de tokens via SSE, retomada explicada, queda→polling graciosa), última análise persistida no cofre com carimbo/selo "dados mudaram", retomada de análise interrompida (checkpoint durável) e saúde financeira em 2 eixos (v2.15, ADR-0023) | `Analise.tsx` + ponte SSE no `main.ts` | ✨ | E2E `analise-linha-do-tempo.spec.ts` (3) + `analise-persistencia.spec.ts` + `tests/test_checkpoint_cofre.py` (13) + `tests/test_analise_ultima.py` (8) + `tests/test_core.py` (2 eixos) |
